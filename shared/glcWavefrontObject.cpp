...
void glcWavefrontObject::ReadPly(char *arquivo){
        ...
        if(buff[0] == 'e' && buff[1] == 'n'){
            ...
            while(fgets(buff, sizeof(buff), file)){
                ...
                if(cont_vertices < num_vertices){
                    ...
                    while(token != NULL){
                        vetor_vertices[indice_linha] = atof(token);
                        indice_linha++;
                        token = strtok(NULL, " ");
            ...
            //termina a execução da funcao para nao ter que ler duas vezes o arquivo inteiro;
            //printf("TESTE: %f", vetor_vertices[0]);
            }
            printf("\n%f", vetor_vertices[0]);
